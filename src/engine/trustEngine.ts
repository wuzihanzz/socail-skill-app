...
const emotionalDisclosureWords = [
  '压力',
  '负担',
  '累',
  '忙',
...
export const calculateTrustDelta = (
  ...
  const asksQuestion = /[?？]/.test(lowerMessage) || hasAny(lowerMessage, engagedQuestionWords);
  const sharesSelf = hasAny(lowerMessage, selfDisclosureWords);
  const sharesFeeling = hasAny(lowerMessage, emotionalDisclosureWords);
  const isSubstantialTurn = compactMessage.length >= 8;

  if (sharesSelf && asksQuestion) {
    return { trustDelta: 2, emotionChange: 'happy', conflictState: 'none' };
  }

  if ((sharesSelf || lowerMessage.includes('我们')) && sharesFeeling) {
    return { trustDelta: 2, emotionChange: 'happy', conflictState: 'none' };
  }

  if (sharesSelf || asksQuestion || isSubstantialTurn) {
    return { trustDelta: 1, emotionChange: 'neutral', conflictState: 'none' };
  }
